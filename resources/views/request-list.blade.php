...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="antialiased">
        <div class="relative flex items-top justify-center min-h-screen bg-linear-gradient(to right top, #514d50, #696469, #807d84, #9896a0, #afb1bd, #abb1bd, #a7b0bc, #a3b0bb, #85949c, #6a797e, #515f61, #3a4645);-100 dark:bg-linear-gradient(to right top, #514d50, #696469, #807d84, #9896a0, #afb1bd, #abb1bd, #a7b0bc, #a3b0bb, #85949c, #6a797e, #515f61, #3a4645);-900 sm:items-center py-4 sm:pt-0">
            ...
            <table id="registrationform" class="display">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $registrationform->name }}</td>
                        <td>{{ $registrationform->email }}</td>
                        <td>{{ $registrationform->number }}</td>
                        <td>{{ $registrationform->bdate }}</td>
                        <td>{{ $registrationform->relaff }}</td>
                        <td>{{ $registrationform->bsdate }}</td>
                        <td>{{ $registrationform->bstime }}</td>                        
                        <td>{{ $registrationform->bslocation }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>   
    ...
    <script>
        jQuery(document).ready(function(){
            jQuery('#registrationform').DataTable();
        });
        </script>
</html>